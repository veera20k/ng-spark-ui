...
@Component({
  ...
  standalone: true,
  template: `
    <div (mouseover)="hoverToggle() && !alwaysOpen() && toggle($event)" (mouseout)="hoverToggle() && cmptLoaderService.close()" (click)="!alwaysOpen() && toggle($event)" id="flash-popover-trigger">
...
export class PopoverComponent {
  ...
  side = input< 'top' | 'bottom' | 'left' | 'right'>('top');
  static = input(false);
